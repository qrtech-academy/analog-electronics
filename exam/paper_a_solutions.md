...
R_B = \frac{5 - 0.7}{15\ \text{mA}} = \mathbf{287\ \Omega}$$

**Nearest E12: 270 ohm.** **(1 mark. The 0.7 V is the rubric's, and it is the figure
`baseResistor` uses: a base driven ten times harder than the active region needs sits above the
0.65 V of a bias point. Using 0.65 gives 290 ohm and the same E12 value, and costs nothing.)**

$$I_B = \frac{5 - 0.7}{270} = \mathbf{15.9\ \text{mA}}, \qquad
\beta_{forced} = \frac{150}{15.9} = \mathbf{9.4}$$
...
A stiffness of 200 needs a divider current of $I_C \times 200/\beta = \mathbf{4.1\ \text{mA}}$,
**four times what the stage amplifies with**. **That is the point at which a designer stops**,
because the bias network is now the dominant power consumer, and uses a Darlington or a MOSFET
input instead so that the base current is negligible in the first place. **(1 mark. Scaling the
present base current instead, $200 \times 18.7\ \mu\text{A} = 3.7$ mA, keeps the mark: it is the
right order and the right conclusion. It is also 8 per cent light, and for (b)'s reason: a stiffer
divider droops less, so $I_C$ climbs back towards 1.06 mA and $I_B$ with it, and 3.7 mA is a
stiffness of 181.)**
