...
book/        The whole course typeset as a book, and the committed PDF.
...
## The Book
The whole course is also typeset as a book: **[Analog Electronics](./book/analog-electronics.pdf)**,
ten chapters and two written papers, roughly 220 pages. It is the same material as the lectures,
with the same figures and the same numbers, arranged to be read front to back rather than followed
week by week.

The PDF in `main` is always the newest build, and each version tag also publishes it as a release.
What it does not contain is the worked solutions to the exercises, which stay here, in
`lectures/LNN/appendix`, so that the book can be read with them out of reach.

Rebuilding it needs LuaLaTeX rather than a compiler:
...
See [book/README.md](./book/README.md) for the layout, the conventions, and how to keep a chapter
in step with the lecture it is typeset from.
...
make diagrams             # Redraw the generated figures. Needs the Python environment.
make book                 # Rebuild the book. Needs LuaLaTeX; see book/README.md.
