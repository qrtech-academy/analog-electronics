...
def _divider_scale_for_stiffness(
    supply: float,
    upper: float,
    lower: float,
    emitter_resistor: float,
    ...
    """The factor both divider legs must be scaled by to reach `stiffness`.

    Scaling the present divider current by the wanted factor is the tempting answer and it
    undershoots, for A.3's reason: a stiffer divider droops less, so the collector current climbs
    back towards its unloaded value and the base current climbs with it. Solving is one line,
    because scaling both legs by k leaves the Thevenin voltage alone and multiplies only the
    Thevenin resistance:

        stiffness = (I_div0 / k) * (k R_th + (hfe + 1) R_E) / (V_th - V_BE)

    Returns 0.0 for a stiffness the divider cannot reach at any size, which is anything at or
    below I_div0 * R_th / (V_th - V_BE): the base current stops falling once the emitter resistor
    rather than the divider dominates what the base sees.
    """
    thevenin_voltage = supply * lower / (upper + lower)
    thevenin_resistance = parallel(upper, lower)
    divider_current = supply / (upper + lower)
    headroom = (
        stiffness * (thevenin_voltage - vbe) / divider_current - thevenin_resistance
    )
    if headroom <= 0.0:
        return 0.0
    return (hfe + 1.0) * emitter_resistor / headroom


def divider_current_for_stiffness(
    supply: float,
    upper: float,
    lower: float,
    emitter_resistor: float,
    ...
    """The divider current a wanted stiffness actually costs.

    Exactly `collector_current * stiffness / hfe`, since the base current is the collector current
    over beta by definition. A stiffness equal to beta therefore costs a divider current equal to
    the collector current, and four times beta costs four times it, which is where a designer
    stops buying stiffness and changes bias arrangement instead.
    """
    scale = _divider_scale_for_stiffness(
        supply, upper, lower, emitter_resistor, stiffness, vbe, hfe
    )
    if scale <= 0.0:
        return 0.0
    return supply / ((upper + lower) * scale)


def bias_error_for_stiffness(
    supply: float,
    upper: float,
    lower: float,
    emitter_resistor: float,
    ...
    """What the stiff-divider prediction still overstates by, once the divider carries that much.

    It never reaches zero. Stiffness attacks the R_th/beta term and nothing else, and what is left
    is the 1/(beta + 1) by which the emitter current exceeds the collector current.
    """
    scale = _divider_scale_for_stiffness(
        supply, upper, lower, emitter_resistor, stiffness, vbe, hfe
    )
    if scale <= 0.0:
        return 0.0
    return loaded_bias_error(
        supply, upper * scale, lower * scale, emitter_resistor, vbe, hfe
    )
