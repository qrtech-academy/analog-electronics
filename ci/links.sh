...
for file in **/*.md; do
    # Third-party trees are somebody else's, and build/ holds assembled copies of files that
    # were already checked at their source. An assembled toolkit is deliberately missing the
    # directory above it, so its README's links out of the tree cannot resolve and are not
    # supposed to.
    case "$file" in
