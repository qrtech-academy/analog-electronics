...
#   8. A heading is followed immediately by what it introduces. CommonMark ends an ATX heading at
#      its newline, so the paragraph, table, figure or fence beneath it begins a new block with or
#      without a blank line between the two, and dropping the blank makes the source group the way
#      the rendered page already does. Verified rather than assumed: every Markdown file here was
#      rendered before and after the blank lines came out, and the HTML is byte for byte the same.
#      A thematic break is exempt, because a "---" sitting directly under a heading reads as a
#      setext underline to a person even though CommonMark parses it as a break.
#   9. A list follows its introducing colon immediately, for rule 8's reason and with one extra
#      condition. A list may interrupt a paragraph only when its first item is non-empty and, if
#      ordered, starts at 1. An ordered list beginning at anything else would change meaning if
#      its blank line were removed, so it keeps the blank and is not reported.
#
# Usage:
#   markdown.sh
set -euo pipefail
...
for file in **/*.md; do
    ...
    # 8. A blank line between a heading and what it introduces. Deciding this needs two lines of
    #    lookahead, which awk has no natural way to express one line at a time, so the file is
    #    read into an array and walked at END. Two blank lines in a row are somebody else's rule
    #    and are left alone; a thematic break under a heading is exempt, as noted at the top.
    while IFS= read -r line; do
        fail "$file:$line: blank line after a heading; the heading owns what follows it"
    done < <(awk '
        { text[NR] = $0 }
        END {
            for (i = 1; i <= NR; i++) {
                if (text[i] ~ /^[[:space:]]*```/) { fence = !fence; continue }
                if (fence)                                            continue
                if (text[i] !~ /^#{1,6} /)                            continue
                if (i + 2 > NR)                                       continue
                if (text[i+1] !~ /^[[:space:]]*$/)                    continue
                if (text[i+2] ~ /^[[:space:]]*$/)                     continue
                if (text[i+2] ~ /^(-{3,}|\*{3,}|_{3,})[[:space:]]*$/) continue
                print i
            }
        }' "$file")

    # 9. A blank line between a line ending in a colon and the list it introduces. Only a list
    #    that could legally interrupt a paragraph is reported: a bullet, or an ordered list
    #    starting at 1. A heading ending in a colon is a heading, and a table row is a table row.
    while IFS= read -r line; do
        fail "$file:$line: blank line between a colon and the list it introduces"
    done < <(awk '
        { text[NR] = $0 }
        END {
            for (i = 1; i <= NR; i++) {
                ...
                if (i + 2 > NR)                       continue
                if (text[i+1] !~ /^[[:space:]]*$/)    continue
                if (text[i+2] ~ /^[[:space:]]*[-*+][[:space:]]/ ||
                    text[i+2] ~ /^[[:space:]]*1\.[[:space:]]/)
                    print i
            }
        }' "$file")
done
