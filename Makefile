...
.PHONY: help test numbers lint links schematic buildable sections markdown tables tables-check \
        format format-check diagrams book clean
...
# Deliberately not part of lint, for the same reason as diagrams: the PDF is committed and
# rebuilding it needs a LuaLaTeX installation this repo does not otherwise require. See
# book/README.md for what to install. The figures come straight from the lecture trees, so
# `make diagrams` redraws the book's figures too.
book: ## Build the book. Optional: VERSION=vx.y.z for the title page.
	$(MAKE) -C $(ROOT)book VERSION=$(VERSION)
