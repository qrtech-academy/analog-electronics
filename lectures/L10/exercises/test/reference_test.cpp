...
[[nodiscard]] constexpr double parallel(const double a, const double b) noexcept
{
    return (a * b) / (a + b);
}

[[nodiscard]] constexpr double intrinsicEmitterResistance(const double current) noexcept
{
    return ThermalVoltage / current;
}

[[nodiscard]] constexpr double outputResistance(const double current) noexcept
{
    return EarlyVoltage / current;
}

[[nodiscard]] constexpr double darlingtonInput(const double current, const double load) noexcept
{
    return Beta * Beta * ((2.0 * intrinsicEmitterResistance(current)) + load);
}

[[nodiscard]] constexpr double darlingtonGain(const double current, const double load) noexcept
{
    return load / ((2.0 * intrinsicEmitterResistance(current)) + load);
}

[[nodiscard]] constexpr double loaded(const double gain, const double source,
                                      const double load) noexcept
{
    return (gain * load) / (source + load);
}

[[nodiscard]] double decibels(const double ratio) noexcept { return 20.0 * std::log10(ratio); }

/** The open-loop gain, with the output stage's beta as the parameter the last test varies. */
[[nodiscard]] double openLoop(const double outputBeta) noexcept
{
