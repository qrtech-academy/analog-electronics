...
[[nodiscard]] double corner(const double resistance, const double capacitance) noexcept
{
    return 1.0 / (2.0 * M_PI * resistance * capacitance);
}

[[nodiscard]] std::complex<double> lowpass(const double frequency, const double cornerHz) noexcept
{
    return 1.0 / std::complex<double>{1.0, frequency / cornerHz};
}

[[nodiscard]] std::complex<double> highpass(const double frequency, const double cornerHz) noexcept
{
