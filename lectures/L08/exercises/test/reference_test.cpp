...
[[nodiscard]] constexpr double loadedGain(const double gain, const double source,
                                          const double load) noexcept
{
    return (gain * load) / (source + load);
}

[[nodiscard]] double baseEmitterVoltage(const double current) noexcept
{
    return ThermalVoltage * std::log(current / SaturationCurrent);
}

/// 2(V_BE + I R_E), with V_BE from the exponential.
[[nodiscard]] double biasFor(const double idle, const double degeneration) noexcept
{
    return 2.0 * (baseEmitterVoltage(idle) + (idle * degeneration));
}

/// The inverse, by bisection in the logarithm. There is no closed form.
[[nodiscard]] double idleFor(const double bias, const double degeneration) noexcept
{
    double low{1.0e-12};
    double high{100.0};
