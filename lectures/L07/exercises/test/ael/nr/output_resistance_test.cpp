...
[[nodiscard]] Netlist stage(const double baseVoltage, const double load,
                            const double emitterResistor, const double forced) noexcept
{
...
[[nodiscard]] double currentAt(const double baseVoltage, const double load,
                               const double emitterResistor) noexcept
{
 ...
 */
[[nodiscard]] double baseVoltageFor(const double load, const double emitterResistor) noexcept
{
    double low{0.40 + (Target * emitterResistor)};
    double high{0.75 + (Target * emitterResistor)};

    for (int iteration{}; iteration < 50; ++iteration)
    {
 ...
 */
[[nodiscard]] double measuredOutputResistance(const double load,
                                              const double emitterResistor) noexcept
{
