...
[[nodiscard]] Netlist pair(const double first, const double second, const Bias& bias) noexcept
{
...
[[nodiscard]] double collector(const double first, const double second, const Bias& bias) noexcept
{
...
[[nodiscard]] double differentialGain(const Bias& bias) noexcept
{
...
[[nodiscard]] double commonModeGain(const Bias& bias) noexcept
{
    return (collector(Step, Step, bias) - collector(-Step, -Step, bias)) / (2.0 * Step);
}

[[nodiscard]] double rejection(const Bias& bias) noexcept
{
...
[[nodiscard]] double tailCurrent(const Bias& bias) noexcept
{
