...
[[nodiscard]] constexpr double sideResistance(const double tail) noexcept
{
    return ThermalVoltage / (tail / 2.0);
}

[[nodiscard]] constexpr double differentialGain(const double load, const double tail) noexcept
{
    return -load / (2.0 * sideResistance(tail));
}

[[nodiscard]] constexpr double commonModeGain(const double load, const double tail,
                                              const double tailResistance) noexcept
{
